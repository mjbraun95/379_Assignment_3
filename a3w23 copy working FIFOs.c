...
#include <sys/socket.h> // For socket-related functions and structs
...
// ###################################################### SERVER
    if (strcmp(argv[1], "-s") == 0) {
        ...
        while (1)
        {
            ...
            for (int idNumber = 1; idNumber < NCLIENT+1; idNumber++) {
                // Check each pollfd for any received events
                if (pollfds[idNumber].revents & POLLIN) 
                {
                    // Event received
                    ...
                    if (received_type == PUT) {
                        ...
                        printf("Transmitted (src= client:%s) (%s: %s)\n", idNumber, received_type, received_message.message);
...
// ###################################################### CLIENT
        ...
        sprintf(fifo_cs_name, "fifo-%d-0", idNumber);
        sprintf(fifo_sc_name, "fifo-0-%d", idNumber);

        // Open 2 FIFOs
        int fifo_cs_fd = open(fifo_cs_name, O_WRONLY | O_NONBLOCK );
        int fifo_sc_fd = open(fifo_sc_name, O_RDONLY | O_NONBLOCK );
